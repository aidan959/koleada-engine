...
public class AidanTesting extends Scene{
    ...
    public void draw(){
        ...
        if(currentFrame < AudioSync.songParts.VERSE1.get() ){
            lightning(width/2f, height/2f,10, map(currentFrame,0, AudioSync.songParts.VERSE1.get(), 0, 100 ), cos(degrees(angle)), true);
            storm(width/4f,height/2, 15,map(currentFrame,0, AudioSync.songParts.VERSE1.get(), 0, 100 ), sin(degrees(angle)), true, 1 , false);    
            storm((width*3)/4f,height/2, 15,map(currentFrame,0, AudioSync.songParts.VERSE1.get(), 0, 100 ), sin(degrees(angle)), true, 0, true);    
            ...
            storm(width/4f,height/2f, 15,map(currentFrame,0, AudioSync.songParts.VERSE1.get(), 0, 100 ), sin(degrees(angle)), true, smoothTubes, false );    
            storm((width*3)/4f,height/2f, 15,map(currentFrame,0, AudioSync.songParts.VERSE1.get(), 0, 100 ), sin(degrees(angle)), true, smoothTubes, true );
        // bridge
        } else if(currentFrame < AudioSync.songParts.CHORUS1.get()){
            pushMatrix();
            stroke(255);
            float circleMap =map(currentFrame,AudioSync.songParts.BRIDGE1.get(),AudioSync.songParts.CHORUS1.get(),0, width);
            if(currentFrame > 4288737){
    ...
    public void lightning(float originX, float originY, int n, float posX,float posY, boolean firstLayer)
    {
        for(int i = 0; i < n; i++)
        {
            ...
            rotateZ(PI/12);
            box(10,100,10);
            if(i == 1)lightning(originX, originY, n - 1,posX, posY,false);
            popMatrix();
        }
    }
    public void storm(float originX, float originY, int n, float posX,float posY, boolean firstLayer, float volume, boolean flip)
    {
        if(firstLayer){
            pushMatrix();
            translate(originX, originY);
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            box(10,100,10);
            if(i == 1)storm(originX, originY, n - 1,posX, posY,false, (volume *9f)/10f, flip);
            popMatrix();
        }
        if(firstLayer){
            popMatrix();
